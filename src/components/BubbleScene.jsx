...
const Bubble = ({ position, delay, scale, onComplete }) => {
...
export const BubbleScene = ({ triggerBurst, mousePosition }) => {
  ...
  useEffect(() => {
    if (triggerBurst > 0) {
      // Add new bubbles on interaction
  ...
  const handleBubbleComplete = (id) => {
    ...
    // Add new bubble if we don't have too many
    setBubbles(prev => {
      if (prev.length < 15) {
        const newBubble = {
          id: bubbleIdRef.current++,
